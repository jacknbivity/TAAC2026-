...
_FALLBACK_MODEL_CFG = {
    ...
    'user_ns_tokens': 0,
    'item_ns_tokens': 0,
    'enable_intraday_calendar_features': True,
    'enable_weekly_calendar_features': True,
    'enable_annual_calendar_features': True,
    'seqContextwakeup': True,
    'enable_seq_reweight': False,
    'seq_reweight_hidden_mult': 2,
    'rank_moe_enable': False,
    'rank_moe_num_experts': 4,
    'rank_moe_top_k': 1,
    'enable_history_time_bias': False,
    'history_time_bias_residual_scale': 1.0,
}
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Special handling for ``num_time_buckets`` and ``num_gap_buckets``: these
    are not exposed on the CLI as independent hyperparameters; the bucket
    counts are uniquely determined by the dataset module. Resolution order:
    ...
    for key in _MODEL_CFG_KEYS:
        if key in ('num_time_buckets', 'num_gap_buckets'):
            use_flag = 'use_time_buckets' if key == 'num_time_buckets' else 'use_gap_buckets'
            bucket_count = NUM_TIME_BUCKETS if key == 'num_time_buckets' else NUM_GAP_BUCKETS
            if key in train_config:
                cfg[key] = train_config[key]
            elif use_flag in train_config:
                cfg[key] = bucket_count if train_config[use_flag] else 0
            else:
                cfg[key] = _FALLBACK_MODEL_CFG[key]
                logging.warning(
                    f"train_config missing both '{key}' and '{use_flag}', "
                    f"using fallback = {cfg[key]}")
            continue

        if key == 'seqContextwakeup' and 'enable_target_sequence_activation' in train_config:
            cfg[key] = train_config['enable_target_sequence_activation']
...
def _batch_to_model_input(
    ...
    # Same lazy-instantiation pattern as ``Trainer._make_model_input``: the
    # dict stays ``None`` unless the dataset side actually emitted the
    # ``{domain}_hist_*_id`` tensors (i.e. the temporal-bias branch is on).
    ...
    for domain in seq_domains:
        ...
        seq_gap_buckets[domain] = device_batch.get(
            ...
            torch.zeros(B, L, dtype=torch.long, device=device))
        # Same probe-then-fan-out trick as in ``Trainer._make_model_input``;
        # the dataset emits all 8 fields atomically when the branch is on.
...
def main() -> None:
    ...
    calendar_feature_switches = {
        'enable_intraday_calendar_features': bool(train_config.get(
            'enable_intraday_calendar_features',
            _FALLBACK_MODEL_CFG['enable_intraday_calendar_features'])),
        'enable_weekly_calendar_features': bool(train_config.get(
            'enable_weekly_calendar_features',
            _FALLBACK_MODEL_CFG['enable_weekly_calendar_features'])),
        'enable_annual_calendar_features': bool(train_config.get(
            'enable_annual_calendar_features',
            _FALLBACK_MODEL_CFG['enable_annual_calendar_features'])),
    }
    use_gap_buckets = bool(
        train_config.get('use_gap_buckets', train_config.get('num_gap_buckets', 0) > 0))
    # Mirror the temporal-bias flag from training so the test dataset emits the
    # matching per-position hour/day/week ids; otherwise the model would build
    # a ``HistoricalTemporalBiasInjector`` it never gets to consume.
    enable_history_time_bias = bool(train_config.get(
        'enable_history_time_bias',
        _FALLBACK_MODEL_CFG['enable_history_time_bias']))
    ...
    test_dataset = PCVRParquetDataset(
        ...
        buffer_batches=0,
        is_training=False,
        use_gap_buckets=use_gap_buckets,
        enable_history_time_bias=enable_history_time_bias,
        **calendar_feature_switches,
    )
